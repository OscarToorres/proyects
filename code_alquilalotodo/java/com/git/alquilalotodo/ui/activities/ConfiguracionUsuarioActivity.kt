...
class ConfiguracionUsuarioActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.confUserSave.setOnClickListener {
            ...
            userDoc.get().addOnSuccessListener {
                ...
                userUpdated = comprobarCampos(userUpdated)
                ...
                if (selectedImage != null) {
                    ...
                    userDoc.set(userUpdated).addOnSuccessListener {
    ...
    private fun comprobarCampos(userUpdated:HashMap<String,Any>) : HashMap<String,Any> {

        if (binding.confNombre.text.isNotBlank()) {
            userUpdated.put("nombre", binding.confNombre.text.toString())
        }
        if (binding.confApellidos.text.isNotBlank()) {
            userUpdated.put("apellidos", binding.confApellidos.text.toString())
        }
        if (binding.confLocalizacion.text.isNotBlank()) {
            userUpdated.put("localizacion", binding.confLocalizacion.text.toString())
        }
        if (binding.confDescripcion.text.isNotBlank()) {
            userUpdated.put("descripcion", binding.confDescripcion.text.toString())
        }
        if (binding.confEmail.text.isNotBlank()) {
            userUpdated.put("email", binding.confEmail.text.toString())
        }
        if (binding.confGenero.text.isNotBlank()) {
            userUpdated.put("genero", binding.confGenero.text.toString())
        }
        if (binding.confBirthday.text.isNotBlank()) {
            userUpdated.put("birthday", binding.confBirthday.text.toString())
        }
        if (binding.confCorreoContacto.text.isNotBlank()) {
            userUpdated.put("correoContacto", binding.confCorreoContacto.text.toString())
        }
        if (binding.confHorasActivo.text.isNotBlank()) {
