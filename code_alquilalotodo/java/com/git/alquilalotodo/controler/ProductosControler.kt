...
class ProductosControler (val auth: FirebaseAuth,val storage : FirebaseStorage,val db: FirebaseFirestore,val context: Context, val activityContext : AppCompatActivity?){
     ...
     */
    fun addProductosFirestore(name:String, description:String, caract:String, precio:String, condic:String,imagenesSelected : ArrayList<Uri>){
        ...
        userDocument.get().addOnSuccessListener {
            if (it.exists()){
                ...
                listProductos.add(Producto(idUser,productId,name,description,condic,caract,"",listUrl,precio))
    ...
    fun actualizarProductosFirestore(name:String, description:String, caract:String, precio:String, condic:String,imagenesSelected : ArrayList<Uri>,posicion:Int,context: AppCompatActivity){
        val currentUser = auth.currentUser
        var listProductos:ArrayList<Producto>
        var listUrl: ArrayList<String> = uploadImageToFirebaseStorage(imagenesSelected)
        val idUser = currentUser!!.uid
        ...
        userDocument.get().addOnSuccessListener {
            if (it.exists()){
                ...
                if (imagenesSelected.isEmpty()){
                    ...
                    val proctoActualizado = Producto(idUser,productoId,name,description,condic,caract,"",listaImagenesRecuperadas,precio)
                    listProductos[posicion] = proctoActualizado
                }else {
                    listProductos[posicion] = Producto(idUser,productoId,name,description,condic,caract,"",listUrl,precio)
                ...
                if (usuario != null) {
                    ...
                    userDocument.update("listaProductos", productos)
                        .addOnSuccessListener {
                            ...
                            /*
                            val fragmento2 = ProfileFragment()

                            //Probablemte falle y hay que cambiarlo
                            val fragmentManager = supportFragmentManager
                            val fragmentTransaction = fragmentManager.beginTransaction()
                            fragmentTransaction.replace(R.id.fragment_main, fragmento2)
